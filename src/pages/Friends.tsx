...
const Friends: React.FC = () => {
  ...
  const searchResults = useMemo(() => {
    ...
    const filteredUsers = usersData.filter(user => {
      ...
      return (
        ...
        (user.university?.name || user.college?.name || "").toLowerCase().includes(searchTerm)
      );
    });
  ...
  const renderContent = () => {
     ...
     switch (activeTab) {
       ...
       case "suggestions":
         return searchQuery.trim() ? renderSearchResults() : <FriendSuggestions />;
       default:
         return searchQuery.trim() ? renderSearchResults() : <FriendsList />;
     }
   };
        ...
        <div className="grid gap-3">
         {searchResults.map((user) => (
           <FriendCard
             ...
             name={user.name}
             avatar={user.avatar}
             university={user.university?.name || user.college?.name || "No University"}
             type={activeTab === "all" ? "friend" : activeTab === "requests" ? "request" : "suggestion"}
           />
         ))}
       </div>
      </div>
  ...
  return (
    <>
      <FriendsHeader searchQuery={searchQuery} onSearchChange={setSearchQuery} />
